...
const currentColor = document.getElementById("colorBox");
const colorButtons = document.getElementById("colorButtons");
const submitColor = document.getElementById("addColorForm");
...
colorButtons.addEventListener('click', function (event) {
  let clickedColor = event.target.className;
    if (clickedColor === "color-btn") {
      clickedColor = event.target.title;
      if (clickedColor === '') {
        clickedColor = event.target.style.backgroundColor;
      }
      currentColor.style.backgroundColor = clickedColor;
      currentColor.innerText = clickedColor.charAt(0).toUpperCase() + clickedColor.slice(1);
    }
});
